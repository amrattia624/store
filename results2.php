  ...
  <body>
    <?php require('header2.php');?>
      <div class="parent-user">
        ...
        <div class="child-user3">الفصل الدراسي </div>
        <?php 
        $select=$r->query("SELECT * FROM $section WHERE 1");
          echo '<div class="child-user5">';
        while($row=mysqli_fetch_array($select)){
            echo '<div class="child-user5-child1">
                    <span>'.$row["first_term"].'</span>
                    <a href="training2.php?id='.$row["description"].'&&name='.$row["first_term"].'&&term=الترم الأول">جرب الان</a>
                  </div>';
        }      
          echo '</div>';
        ?>   
